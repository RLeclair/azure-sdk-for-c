...
static void _az_pahoasync_critical_error() { az_platform_critical_error(); }

static void _az_pahoasync_on_disconnect(void* context, char* cause)
{
  (void)context;
  (void)cause;
}

static int _az_pahoasync_on_message(
    void* context,
    char* topicName,
    int topicLen,
    MQTTAsync_message* m)
...
static void _az_pahoasync_on_connect_success(void* context, MQTTAsync_successData5* response)
{
  az_result ret;
  az_mqtt5* me = (az_mqtt5*)context;
...
static void _az_pahoasync_on_connect_failure(void* context, MQTTAsync_failureData5* response)
{
  az_result ret;
  az_mqtt5* me = (az_mqtt5*)context;
...
static void _az_pahoasync_on_subscribe_success(void* context, MQTTAsync_successData5* response)
{
  az_mqtt5* me = (az_mqtt5*)context;
...
static void _az_pahoasync_on_subscribe_failure(void* context, MQTTAsync_failureData5* response)
{
  (void)context;
  (void)response;
...
AZ_INLINE az_result _az_result_from_pahoasync(int pahoasync_ret)
{
  az_result ret;
...
static void _az_pahoasync_on_disconnect_failure(void* context, MQTTAsync_failureData5* response)
{
  (void)context;
  (void)response;
...
az_mqtt5_outbound_connect(az_mqtt5* mqtt5, az_mqtt5_connect_data* connect_data)
{
  ...
  char server_uri_buffer[connect_data->host._internal.size + SERVER_URI_COLON_LENGTH + SERVER_URI_PORT_MAX_LENGTH + 1 ];
  ...
  _az_RETURN_IF_FAILED(_az_result_from_pahoasync(MQTTAsync_createWithOptions(
  ...
  _az_RETURN_IF_FAILED(_az_result_from_pahoasync(MQTTAsync_setCallbacks(
      *me->_internal.pahoasync_handle,
      mqtt5,
      _az_pahoasync_on_disconnect,
      _az_pahoasync_on_message,
      NULL)));

  conn_opts.keepAliveInterval = AZ_MQTT5_DEFAULT_MQTT_CONNECT_KEEPALIVE_SECONDS;
  conn_opts.cleansession = false;
  conn_opts.onSuccess5 = _az_pahoasync_on_connect_success;
  conn_opts.onFailure5 = _az_pahoasync_on_connect_failure;
  conn_opts.context = me;

  _az_RETURN_IF_FAILED(
      _az_result_from_pahoasync(MQTTAsync_connect(*me->_internal.pahoasync_handle, &conn_opts)));
...
AZ_NODISCARD az_result az_mqtt5_outbound_sub(az_mqtt5* mqtt5, az_mqtt5_sub_data* sub_data)
{
  ...
  opts.onSuccess5 = _az_pahoasync_on_subscribe_success;
  opts.onFailure5 = _az_pahoasync_on_subscribe_failure;
  opts.context = mqtt5;

  return _az_result_from_pahoasync(MQTTAsync_subscribe(
...
AZ_NODISCARD az_result az_mqtt5_outbound_pub(az_mqtt5* mqtt5, az_mqtt5_pub_data* pub_data)
{
  ...
  opts.onSuccess5 = _az_pahoasync_on_publish_success;
  opts.onFailure5 = _az_pahoasync_on_publish_failure;
  opts.context = mqtt5;
  ...
  return _az_result_from_pahoasync(MQTTAsync_sendMessage(
...
AZ_NODISCARD az_result az_mqtt5_outbound_disconnect(az_mqtt5* mqtt5)
{
  ...
  opts.onSuccess5 = _az_pahoasync_on_disconnect_success;
  opts.onFailure5 = _az_pahoasync_on_disconnect_failure;
  opts.context = mqtt5;

  return _az_result_from_pahoasync(MQTTAsync_disconnect(*mqtt5->_internal.pahoasync_handle, &opts));
}
    ...
    az_mqtt5_property_string* prop_str)
{
  ...
  return _az_result_from_pahoasync(
    ...
    uint32_t prop_int)
{
  ...
  property_bag->pahoasync_property.identifier = (enum MQTTPropertyCodes)type;
  property_bag->pahoasync_property.value.integer4 = prop_int;
